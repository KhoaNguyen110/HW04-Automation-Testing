...
//   test("TC-02: Chưa đăng nhập nhấn thanh toán - Hiển thị popup và Redirect về /login", async ({
//     page,
//   }) => {
//     // 1. Vẫn cho phép thêm sản phẩm vào giỏ khi chưa đăng nhập
//     await addSingleItemFromHome(page);
//     await page.getByRole("link", { name: "Giỏ hàng" }).click();

//     // 2. Thiết lập Listener để bắt popup TRƯỚC KHI nhấn nút thanh toán
//     // Lưu ý: Playwright tự động auto-dismiss dialog nếu không có listener
//     page.on("dialog", async (dialog) => {
//       // Kiểm tra nội dung popup
//       expect(dialog.message()).toContain("Bạn cần đăng nhập để thanh toán");
//       // Nhấn nút "OK" trên popup
//       await dialog.accept();
//     });

//     // 3. Nhấn nút để trigger popup
//     await page.getByRole("button", { name: "Tiến hành thanh toán" }).click();

//     // 4. Kiểm tra hệ thống có redirect về trang login sau khi nhấn OK hay không
